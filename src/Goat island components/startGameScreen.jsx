...
import { GiScrollQuill, GiTiedScroll } from "react-icons/gi";
...
const StartGameScreen = () => {
    ...
    return (
        <>
            <div className={'inset-0 bg-black fixed w-full h-full flex items-center justify-center overflow-hidden bg-center bg-no-repeat bg-cover'} style={{backgroundImage: `url(${Background})`}}>
                {/*POSITION CHARACTERS ON SCREEN*/}
                <img src={Boy} alt={'Boy'} className={'absolute left-2 bottom-0 h-[28%] sm:h-[35%] lg:h-[45%] drop-shadow-xl'} />
                <img src={Girl} alt={'Girl'} className={'absolute right-4 bottom-0 h-[28%] sm:h-[35%]s lg:h-[45%] drop-shadow-xl'} />
                <div className={'inset-0 relative w-[90%] w-max-[535px] h-[655px] flex flex-col items-center justify-center'}>
                    {/*h-[655px]*/}
                    <img src={Scroll} alt={'ScrollBackground'} className={'h-full w-full absolute inset-0 object-contain drop-shadow-2xl'} style={{ transform: 'rotate(90deg) scale(1.3)' }} />
                    <div className={'flex flex-col relative items-center mt-0 z-20 px-2 sm:px-6'}>
                        ...
                        <div className={'flex gap-10 sm:gap-8 mb-6'}>
                            <GiScrollQuill onClick={() => setDescription('DictionaryDescription')}
                        ...
                        <div className={'flex flex-col gap-2 mt-10 sm:mt-8'}>
                            ...
                            <button className={`text-2xl sm:text-xl rounded-full 
