...
export const AnimalFactDetail = ({animal, Close, onBack, content, language = "english"}) => {
    const [fact] = useState(() => {
        // Choose English or French facts based on language
        const facts = language === 'french' && animal.FactsFrench ? animal.FactsFrench : animal.Facts;
        if(facts.length === 0){
            return language === 'french' ? "Aucun fait disponible pour cet animal" : "No facts available for this animal";
        }
    ...
    return (
        <div data-cy={"animal-fact-detail"} className={'inset-0 w-screen h-screen absolute flex justify-center items-center bg-no-repeat bg-cover bg-center overflow-hidden'} style={{backgroundImage:`url(${IslandBackground})`}}>
            ...
            <div className={'absolute border border-solid bg-yellow-100/100 border-amber-400 px-2 py-4 z-20 rounded-lg shadow-inner  shadow-amber-400'}>
                <h3 className="text-2xl font-bold mb-4">{animalName} {content.animalFact}</h3>
                <p className="mb-6 italic">"{fact}"</p>
                <div className="flex justify-end space-x-3">
                    <button
                        data-cy={'animal-detail-back'}
                        onClick={onBack}
                        className={'px-4 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 hover:-translate-y-2'}>{content.backToAnimals}</button>
                    <button
                        data-cy={'animal-detail-continue'}
                        onClick={Close}
                        className={'px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 hover:-translate-y-2'}>{content.continueGameFromFact}</button>
                </div>
            </div>
